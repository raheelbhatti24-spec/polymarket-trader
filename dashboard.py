...
# Build table rows separately to avoid f-string issues
table_rows = ''
if recent:
    for t in recent:
        trade_id = t['trade_id']
        ...
        entry = t['entry_price']
        exit_val = t.get('exit_price', 0)
        pnl = t.get('pnl', 0)
        badge_class = 'win' if pnl >= 0 else 'loss'
        table_rows += f'<tr><td>#{trade_id}</td><td>{question}</td><td>${entry:.4f}</td><td>${exit_val:.4f}</td><td><span class="badge {badge_class}">${pnl:+.2f}</span></td></tr>'
else:
    table_rows = '<tr><td colspan="5" style="text-align:center;color:#999">No trades yet</td></tr>'
...
<style>
body{{font-family:Arial;background:linear-gradient(135deg,#667eea,#764ba2);padding:20px;color:white;margin:0}}
...
<div class="card">
<h2>Total P&L</h2>
<div class="stat {pnl_class}">${total_pnl:+.2f}</div>
</div>
...
<p style="text-align:center;opacity:0.8;margin-top:30px">Last Updated: {now_time}</p>
</div>
</body></html>'''
